 ...
 *
 * File:        nfm_sample_ntuple_modify.c
 * Description: Sample wire application using the packet API, and the ntuple
 *              flow modification API demonstrating how to reject HTTP traffic.
 */
...
int process_packet(ns_packet_device_h dev, ns_packet_t* pckt) {
  ns_nfm_ret_t ret;
  ...
  if (pckt->packet_length >= (ETH_HLEN+20+20)) {
    ...
    if (ip->version==4 && ip->protocol==IPPROTO_TCP) {
      ...
      if (ntohs(tcp->dest)==80) {
        ...
        ret=ns_flow_create_ntuple_modifier(dev, ip->saddr, ip->daddr, tcp->source, tcp->dest, IPPROTO_TCP, 0, &fm);
        if (ret==NS_NFM_SUCCESS) {
          ns_flow_reject(fm);
          ns_flow_apply_modifier(fm);
          return 0;
        } else {
          print_error(ret, "ns_flow_create_ntuple_modifier");
        }
      }
    }
  }
